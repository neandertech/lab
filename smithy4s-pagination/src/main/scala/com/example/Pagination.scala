          ...
          endpoint.hints match {
            case Paginated.hint(Paginated(Some(inputToken), Some(outputToken), _, _)) =>
              paginate[F, Input, Output, String](
                ...
                getTokenFromOutput = endpoint.output.compile(new GetTokenVisitor(outputToken.value)),
                putTokenInInput = endpoint.input.compile(new SetTokenVisitor(inputToken.value))
              )
